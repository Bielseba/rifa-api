...
router.get('/', adminViewRequired, async (req, res, next) => {
  try{
    const { rows } = await pool.query('SELECT id, title, status, ticket_price, total_tickets, draw_date FROM campaigns ORDER BY id DESC LIMIT 50');
...
router.post('/campaigns/new', adminViewRequired, async (req, res, next) => {
  try{
    ...
    if (auto_generate) {
      ...
      await pool.query('SELECT admin_generate_tickets($1,$2)', [created.id, d]);
    }
    return res.redirect(`/admin-ui/campaigns/${created.id}/numbers`);
  }catch(e){
...
router.get('/campaigns/:id/numbers', adminViewRequired, async (req, res, next) => {
  try{
    ...
    const campQ = await pool.query('SELECT * FROM campaigns WHERE id=$1', [id]);
    if(!campQ.rowCount) return res.redirect('/admin-ui');
    const numbersQ = await pool.query('SELECT * FROM admin_list_numbers($1)', [id]);
...
router.post('/campaigns/:id/reserve', adminViewRequired, async (req, res, next) => {
  try{
    ...
    await pool.query('SELECT * FROM admin_reserve_ticket($1,$2,$3)', [id, ticket_number, minutes || 30]);
    res.redirect(`/admin-ui/campaigns/${id}/numbers`);
  }catch(e){ next(e); }
});
...
router.post('/campaigns/:id/sell', adminViewRequired, async (req, res, next) => {
  try{
    ...
    await pool.query('SELECT * FROM admin_sell_ticket_manual($1,$2,$3,$4,$5)', [id, ticket_number, customer_name || null, customer_phone || null, req.admin?.id || null]);
    res.redirect(`/admin-ui/campaigns/${id}/numbers`);
  }catch(e){ next(e); }
});
